...
/**
 * This class contains solutions to Question 5 b.
 */
public class Viterbi
{
    ...
    public void init()
    {
        ...
        //emiPara = bTagger.compEmiPara("/Users/lee/Dropbox/NLP/Problem Set 1/Programming/ner.rare.counts");
        emiPara = bTagger.compEmiPara("./ner.rare.counts");
        /** Retrieve transitional probability **/
        HMMTagger hTagger = new HMMTagger();
        //tranProb = hTagger.compTranProb("/Users/lee/Dropbox/NLP/Problem Set 1/Programming/ner.rare.counts");
        tranProb = hTagger.compTranProb("./ner.rare.counts");
    }
    ...
    /**
     * Output the tagged sentences in a file, named ViterbiTagged.
    ...
    public void writeResultToFile(ArrayList<ViterbiReturn> results)
    {
        ...
        try
        {
            writer = new PrintWriter("ViterbiTagged", "UTF-8");
    ...
    public ViterbiReturn act(String sentence)
    {
        ...
        if(temp.length == 1)
        {
            ...
            double prob = (emiPara.get(sentence) == null) ? mt.getMax(emiPara.get("_RARE_").tags).prob : mt.getMax(emiPara.get(sentence).tags).prob;
            String tag = (emiPara.get(sentence) == null) ? mt.getMax(emiPara.get("_RARE_").tags).tag : mt.getMax(emiPara.get(sentence).tags).tag;
            viterbiResult.add(tag);
            viterbiProb.add(prob);
        ...
        for(k = 2; k < sen.length-1; k++)
            for (int u : chooseSet(k - 1))
                for (int v : chooseSet(k))
                {
                    ...
                    {
                        double emiProb = (emiPara.get(sen[k]) == null) ? emiPara.get("_RARE_").tags.get(e.get(v)).prob : emiPara.get(sen[k]).tags.get(e.get(v)).prob;
    ...
    public static void main(String[] args)
    {
        ...
        /** Output results to file, named ViterbiTagged **/
